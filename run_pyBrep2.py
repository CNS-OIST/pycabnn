...
co = [] # To make a KDtree to search
for i in range(0, len(MF_GL_temp)):
    co.append(MF_GL_temp[i][1])
tree = spatial.cKDTree(points).query(co, k=1)[1] #It can quickly search where is the glomeruli index from the coodinates
MF_GL = np.zeros((len(MF_GL_temp), 3))
for k in range(0, len(MF_GL_temp)):
    MF_GL[k, 0] = MF_GL_temp[k][0] # Source index
    MF_GL[k, 1] = tree[k] # Target index
...
co = [] # To make a KDtree to search
for i in range(0, len(GRC_GL_temp)):
    co.append(GRC_GL_temp[i][1])
tree_grc = spatial.cKDTree(points).query(co, k=1)[1] #It can quickly search where is the glomeruli index from the coodinates
GRC_GL = np.zeros((len(GRC_GL_temp), 3))
for k in range(0, len(GRC_GL_temp)):
    GRC_GL[k, 0] = GRC_GL_temp[k][0] # Source index
    GRC_GL[k, 1] = tree_grc[k] # Target index
...
for i in range(0, len(goc_points)):  # Go to each central point
    ...
    if len(new_MGoc) != 0:
        ...
        for j in new_ind:
            Goc_MF_temp.append((i, new_MGoc[j])) # Now, we can recognize which number of goc_points connects with glomerulus
co = [] # To make a KDtree to search
for i in range(0, len(Goc_MF_temp)):
    co.append(Goc_MF_temp[i][1])
tree_goc = spatial.cKDTree(MF).query(co, k=1)[1] #It can quickly search where is the MF index from the coodinates
GOC_MF = np.zeros((len(Goc_MF_temp), 3))
for k in range(0, len(Goc_MF_temp)):
    GOC_MF[k, 0] = Goc_MF_temp[k][0] # Source index
    GOC_MF[k, 1] = tree_goc[k] # Target index
    GOC_MF[k, 2] = distance_test(goc_points[Goc_MF_temp[k][0]], Goc_MF_temp[k][1]) # Distance between the source and target
del Goc_MF_temp
np.savetxt('GOC_MF.txt', GOC_MF)
print('Complete GOC_MF')
