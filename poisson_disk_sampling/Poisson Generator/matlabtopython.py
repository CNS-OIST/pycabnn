...
def Bridson_sampling(sizeI, spacing, nPts, showIter, pts1):
    ...
    while ptsCreated < nPts and nEmptyGrid > 0:
        ...
        D = D[:, 1]
        D1, _ = spatial.KDTree(np.vstack((pts1[:, 1].flatten(), tempPts[:, 1].flatten()))).query(tempPts[:, 1], k=2)  # To make glomeruli's y axis longer
        D1 = D1[:, 1]
        ...
        eligiblePts = (withinI > 0) * (D > spacing)*(D1 > 58)
...
if __name__ == '__main__':
    ...
    pts1 = np.random.rand(30, 3)
    points1 = Bridson_sampling((700, 1500, 430), 20, 230, True, pts1)
