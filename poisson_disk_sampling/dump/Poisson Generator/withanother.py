...
def Bridson_sampling(sizeI=(1, 1), spacing=0.005, nPts=5000, showIter=True, pts1):
    ...
    while ptsCreated < nPts and nEmptyGrid > 0:
        ...
        D, _ = spatial.KDTree(np.vstack((pts1, pts, tempPts))).query(tempPts, k=2)
        D1,_ = spatial.KDTree(np.vstack((pts1[:,1], tempPts[:, 1]))).query(tempPts[:, 1], k=2)  # To make glomeruli's y axis longer
        ...
        eligiblePts = (withinI > 0) * (D > spacing) * (D1 > 55)
