...
def Bridson_sampling(width=100, depth=100, height=100, radius=5, k=600):
    ...
    def in_limits(p):
        return 0 <= p[0] < width and 0.5 <= p[1] < depth and 0 <= p[2] < height
    ...
    #Main process
    points = []
    add_point((width/2, depth/1.5, height/2))
    count = 1
    print('point initialization complete')
    reject = 0
    reject_count = []
    count_count = []
    elapsed_time = []
    ...
    while (count < k):
       ...
       # else:
            #break
    return P[M], elapsed_time, count_time
...
if __name__ == '__main__':
    ...
    # Data save
    #np.savetxt('Multipy point 3.txt', points1)
