...
class Connect_3D(object):
    ...
    def connections_parallel(
        ...
        lam_qpt = lambda ids: parallel_util.find_connections_3dpar(
        ...
        if not deparallelize:
            ...
            id_ar = [
                np.arange(id_sp) + i * id_sp for i in range(int(np.ceil(nqpts / id_sp)))
            ]
...
class Connect_2D(object):
    ...
    def connections_parallel(
        ...
        if type(req_lin_in_tree).__name__ == "bool":
            kdt, q_pts = self.get_tree_setup(
                return_lax=False, lin_in_tree=req_lin_in_tree
            )
        else:
        ...
        lam_qpt = lambda ids: parallel_util.find_connections_2dpar(
            kdt, pts, lpts, c_rad, lin_axis, lin_in_tree, lin_is_src, ids, debug
        )
    ...
    def find_connections(self, lin_in_tree=[]):
        ...
        for i, pt in enumerate(q_pts):  # iterate through the query points
            ...
            if self.lin_in_tree:
                ...
                l_res.append(
                    abs(lax_c[i] - lax_range[ind, 0] - self.lpts.set_0[ind])
                    + self.lpts.lin_offset[ind]
                )
            else:
                ind = ind[
                    np.logical_and(
                ...
                l_res.append(
                    abs(lax_c[ind].ravel() - lax_range[i, 0] - self.lpts.set_0[i])
                    + self.lpts.lin_offset[i]
                )
    ...
    def save_result(self, prefix="", table_name="connection", save_mode="hdf5"):
        ...
        if save_mode == "hdf5":
            foutname = prefix + ".h5"
            store = pd.HDFStore(
                foutname, "w", complevel=1, data_columns=["source", "target"]
            )
