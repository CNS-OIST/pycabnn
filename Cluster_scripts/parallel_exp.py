...
#Output path
global_prefix = './output/parallel_full/'
#config files
...
with dv.sync_imports(): # note: import as does not work as only import part works, not assignment. Also, you will have to store the file in a directory reachable from the PYTHONPATH
    ...
    #import numpy
dv.push(dict(kdt = kdt_aa, q_pts = q_pts_aa, c_rad = c_rad_aa, lax_c = lax_c, lax_range = lax_range, lin_in_tree = lin_in_tree, prefix = prefix, pts = gg.qpts))

#lam_qpt_aa = lambda pt: BREPpy.pt_in_tr2(kdt_aa, pt, c_rad_aa)
#lam_qpt_pf = lambda pt: BREPpy.pt_in_tr2(kdt_pf, pt, c_rad_pf)
lam_qpt = lambda ids: parallel_util.pts_in_tr_ids (kdt, q_pts, lax_c, lax_range, ids, lin_in_tree)
dv.block = False
...
spacing = np.ceil(len(q_pts_aa)/len(rc.ids))

id_ar = get_id_array(len(q_pts_aa), spacing)
